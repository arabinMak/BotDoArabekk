...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Salve @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* Ou se apresenta ou mama o Arabe`
				...
				teks = `🤬ala saiu e nem pagou a coca🤬 @${num.split('@')[0]} até quando isso ?`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
