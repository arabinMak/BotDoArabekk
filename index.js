...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Salve @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}*Se apresenta ou mama o Arabe`
				...
				teks = `🤬ala mais saiu e não pagou a coca🤬 @${num.split('@')[0]} até quando isso ?`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬🤬❭Espera um pouco aí irmão❬🤬❭',
				success: 'toma ai mlk chato/a',
				error: {
				...
				only: {
					...
					ownerG: '❬🤬❭ SÓ O ARABE PODE USAR ESSE COMANDO FI KK.❬🤬❭ ',
					ownerB: '❬🤬❭  Só O ARABE PODE USAR ESSE COMANDO FI KK.❬🤬❭  ',
					admin: ' ❬ 🥴 ❭ ALA O CARA ACHA QUE É ADM KKK ❬ 🥴 ❭ ',
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('me deixa dormir mano')
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Dj azeitona é melhor mano kk'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('safadao kkkk tem que botar o nome né')
					reply(mess.wait)
                ...
                case 'logo3d':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Tapoha é poema nao mano kkkk', text, {quoted: mek})
			...
			case 'mapa': 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`tenta fazer certo ai mano`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
				...
				case 'tiktokperfil':
					try {
						if (args.length < 1) return client.sendMessage(from, 'quem é o tik toker ?kkk ', text, {quoted: mek})
				...
				case 'clearall':
				case 'limpar':
					if (!isOwner) return reply(' te amo arabi😛')
				...
				case 'adicionar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(`mais Ja abriu o grupo? nem descansei`)
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'toimg':
				case 'converter':
					if (!isQuotedSticker) return reply('tem q botar na figura mano kkk burrao'')
					reply(mess.wait)
